...
(function(){
  ...
  function install(){try{var w=frame.contentWindow,doc=w&&w.document;if(!w||!doc||!doc.body)return;if(doc.__studentHomeworkBridgeInstalled)return;doc.__studentHomeworkBridgeInstalled=true;
    /* Directly hook the application's tab API. This is more reliable than relying only on DOM click interception. */
    if(typeof w.openStudentTab==='function' && !w.__studentHomeworkOpenTabHook){
      ...
      w.openStudentTab=function(tab){
        if(String(tab)==='Homework'){
          w.state.studentTab='Homework';
          w.state.page='studentTab';
          return renderHomeworkTab(w);
        }
...
/*
 * SECONDARY HARDENING LAYER
 * The master page owns the curriculum controller; the student page lives in the iframe.
 * This layer keeps the Student Homework tab on the central queue even when the iframe
 * has already rendered before the bridge's first load callback.
 */
(function(){
  'use strict';
  var frame=document.getElementById('app') || document.getElementById('appFrame');
    ...
    try{
      ...
      w.openStudentTab=function(tab){
        if(String(tab)==='Homework'){
          ...
          var id=String(w.state.sid||'');
          var rec=C&&C.getHomework?C.getHomework(id):null;
          var items=rec&&Array.isArray(rec.items)?rec.items:[];
          var s=w.data&&w.data.students&&w.data.students.find(function(x){return String(x.id)===id});
          if(!s)return;
          ...
          var body='<div class="title"><h2>🏠 HOMEWORK · '+esc(s.name)+'</h2><button class="btn ghost" onclick="student()">← Daily Planner</button></div>'+
            ...
            (items.length?items.map(function(x){return '<div class="repcard mintbg"><b>🎼 '+esc(x.title||x.name||'Selected item')+'</b><div class="small">'+esc(x.category||'Homework')+(x.curriculum?' · '+esc(x.curriculum):'')+'</div></div>';}).join(''):'<div class="empty">No homework for this student yet.</div>')+'</div>';
          w.shell(body);
          return true;
        }
  ...
  frame.addEventListener('load',function(){setTimeout(hook,0);setTimeout(hook,100);setTimeout(hook,500)});
