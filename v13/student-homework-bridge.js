/* VIOLIN AI V13 — Student Homework Mirror
   ...
   IMPORTANT: this bridge only adapts the Student Homework function.
   It must never intercept DOM clicks. The Student tab's native onclick remains responsible for navigation. */
(function(){
'use strict';
...
function controller(){try{return window.VIOLIN_AI_CURRICULUM&&window.VIOLIN_AI_CURRICULUM.controller}catch(e){return null}}
...
'<button class="tab" data-student-tab="Pieces">🎼 REPERTOIRE</button><button class="tab" data-student-tab="Scales">🎵 SCALES</button><button class="tab" data-student-tab="Études">📚 ÉTΟΥΔΕΣ</button><button class="tab" data-student-tab="Technical Studies">🧩 TECHNIQUE</button><button class="tab active" data-student-tab="Homework">🏠 HOMEWORK</button></div><div class="card"><div class="section-head"><h3>📚 CENTRAL HOMEWORK</h3><span class="pill">'+a.length+' selected</span></div><p class="small">Live mirror — Central Homework is the single source of truth.</p>'+
...
function hook(){try{var w=getW();if(!w||!w.document||!w.document.body)return false;if(w.__centralHomeworkMirrorHook)return true;var original=w.openStudentTab;if(typeof original!=='function')return false;w.openStudentTab=function(tab){if(String(tab)==='Homework')return openHomework();return original.apply(w,arguments)};w.__centralHomeworkMirrorHook=true;return true}catch(e){return false}}
