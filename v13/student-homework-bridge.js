...
(function(){
  ...
  function install(){try{var w=frame.contentWindow,doc=w&&w.document;if(!w||!doc||!doc.body)return;if(doc.__studentHomeworkBridgeInstalled)return;doc.__studentHomeworkBridgeInstalled=true;doc.addEventListener('click',function(e){var el=e.target&&e.target.closest?e.target.closest('button'):null;if(!el)return;var t=clean(el.textContent).toUpperCase();if(t!=='🏠 HOMEWORK'&&t!=='HOMEWORK')return;if(!el.closest('.tabs'))return;if(w.state&&w.state.sid){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();renderHomeworkTab(w);}},true);}catch(e){}}
