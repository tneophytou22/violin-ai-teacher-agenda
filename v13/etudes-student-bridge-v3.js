/* V13 ÉTUDES STUDENT BRIDGE — independent from SCALES */
(function(){
'use strict';
var frame=document.getElementById('app');
if(!frame)return;
var STORE='VIOLIN_AI_CURRICULUM_SELECTION_V2',HOME='VIOLIN_AI_HOMEWORK_QUEUE_V1',ACTIVE='VIOLIN_AI_LESSON_ACTIVE_V13',PREFIX='VIOLIN_AI_ETUDE_SELECTION_STUDENT_V1_';
...
function currentStudent(d){
 ...
 var titleEls=d.querySelectorAll('h1,h2,h3,h4,.title,.page-title,.student-title,[class*="title"]');
 for(var i=0;i<titleEls.length;i++){
  var raw=text(titleEls[i].textContent),m=raw.match(/(?:ÉTUDES|ETUDES)\s*[·•—-]\s*(.+)$/i);
  if(m){var n=text(m[1]).toLowerCase();var exact=students().find(function(s){return studentName(s).toLowerCase()===n});if(exact)return exact;var part=students().find(function(s){var sn=studentName(s).toLowerCase();return sn&&n&&(sn.indexOf(n)>=0||n.indexOf(sn)>=0)});if(part)return part}
 }
 return null;
}
function isEtudesPage(d){
 var els=d.querySelectorAll('h1,h2,h3,h4,.title,.page-title,.student-title,[class*="title"]');
 for(var i=0;i<els.length;i++)if(/(?:ÉTUDES|ETUDES)/i.test(text(els[i].textContent)))return true;
 var tabs=d.querySelectorAll('.tabs .tab.active,.tab.active,button.active');
 for(var j=0;j<tabs.length;j++)if(/(?:ÉTUDES|ETUDES)/i.test(text(tabs[j].textContent)))return true;
 return false;
}
function itemsFor(s){
...
function save(s,items,kind){
 ...
 var p={studentId:id,studentName:studentName(s),level:levelOf(s),term:termOf(s),kind:kind,curriculum:'etudes',items:items,updatedAt:new Date().toISOString(),source:'V13 Etudes Bridge v5'};
...
function render(d){
 ...
 host.querySelector('[data-a="lesson"]').onclick=function(){var x=chosen();if(!x.length)return alert('Select at least one étude.');var p=save(s,x,'lesson');if(!p)return alert('Could not save the Études selection.');write(PREFIX+sid(s),x);var u=new URL('./lesson-v13.html',window.location.href);u.searchParams.set('studentId',p.studentId);u.searchParams.set('v','etudes-to-lesson-v5-'+Date.now());window.top.location.assign(u.href)};
 host.querySelector('[data-a="homework"]').onclick=function(){var x=chosen();if(!x.length)return alert('Select at least one étude.');var p=save(s,x,'homework');if(!p)return alert('Could not save the Études Homework selection.');write(PREFIX+sid(s),x);alert('Selected études added to Homework.')};
}
function sync(){var d=doc();if(d&&d.body)try{render(d)}catch(e){console.error('[V13 Etudes v5]',e)}}
...
window.VIOLIN_AI_ETUDES_BRIDGE={version:'5.0',sync:bind};
})();
