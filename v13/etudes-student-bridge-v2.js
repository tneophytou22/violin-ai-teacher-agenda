...
function currentStudent(d){
 ...
 for(const el of d.querySelectorAll('h1,h2,h3,h4,.title,.page-title,.student-title,[class*="title"]')){
  const raw=text(el.textContent),m=raw.match(/(?:ÉTUDΕS|ÉTUDES|ETUDES)\s*[·•—-]\s*(.+)$/i);if(!m)continue;
  const n=text(m[1]).toLowerCase();let s=ss.find(x=>text(x.name||x.studentName).toLowerCase()===n);if(s)return s;s=ss.find(x=>{const sn=text(x.name||x.studentName).toLowerCase();return sn&&n&&(sn.includes(n)||n.includes(sn))});if(s)return s;
 }
 return null;
}
function isEtudesPage(d){
 const els=d.querySelectorAll('h1,h2,h3,h4,.title,.page-title,.student-title,[class*="title"]');
 for(const el of els){if(/(?:ÉΤUDΕS|ÉTUDΕS|ÉTUDES|ETUDES)/i.test(text(el.textContent)))return true}
 return [...d.querySelectorAll('.tab.active,button.active')].some(x=>/(?:ÉΤUDΕS|ÉΤUDΕS|ÉTUDES|ETUDES)/i.test(text(x.textContent)));
}
function itemsFor(s){
 const l=levelOf(s),t=termOf(s),D=w.VIOLIN_ETUDES_CURRICULUM_V1,c=D?.levels?.[l]?.terms?.[t];
...
function sync(){try{const d=frame.contentDocument;if(d?.body)render(d)}catch(e){console.error('[V13 Etudes v3] sync error',e)}}
...
frame.addEventListener('load',()=>{bound=null;lastKey='';setTimeout(bind,0);setTimeout(bind,150);setTimeout(bind,500)});if(frame.contentDocument)bind();w.VIOLIN_AI_ETUDES_BRIDGE={version:'3.0',sync:bind};
})(window);
