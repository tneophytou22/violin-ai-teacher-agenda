...
/* V13 CURRICULUM BRIDGE — canonical transport layer.
   Architecture lock:
   UI selection -> canonical V2 store -> explicit lesson handoff -> Lesson.
   The handoff is a transport snapshot, not a second curriculum database.
   Lesson rendering never reads the full curriculum; it reads only the selected snapshot.
*/
...
function students(){try{const x=JSON.parse(localStorage.getItem('VIOLIN_AI_AGENDA_V10')||'{}');return Array.isArray(x.students)?x.students:Array.isArray(x)?x:[]}catch(e){return[]}}
...
function curriculumItems(s){
 const C=window.parent.VIOLIN_AI_CURRICULUM?.controller;
 if(C&&typeof C.list==='function'){const a=C.list('scales',Number(s.level)||1,Number(s.term)||1);if(Array.isArray(a)&&a.length)return a}
 const D=window.parent.VIOLIN_SCALE_CURRICULUM_V1,c=D?.[s.level]?.[Number(s.term)||1]||D?.[s.level]?.[1];if(!c)return[];
 const groups=[['major','Major Scales'],['minor','Minor Scales'],['arpeggios','Tonic Arpeggios'],['dominant7','Dominant 7th'],['diminished7','Diminished 7th'],['chromatic','Chromatic Scales'],['doubleStops','Double Stops'],['oneString','One-String Scales']],out=[];
 groups.forEach(([k,cat])=>(Array.isArray(c[k])?c[k]:[c[k]]).forEach(x=>x&&out.push({id:'scale-'+s.level+'-'+(Number(s.term)||1)+'-'+out.length,curriculum:'scales',category:cat,title:String(x),level:Number(s.level)||1,term:Number(s.term)||1,requirements:{tempo:c.tempo,octaves:c.octaves,bowing:c.bowing,articulation:c.articulation,rhythmicGroups:c.rhythmicGroups,accents:c.accents,dynamics:c.dynamics,positions:c.positions},source:'scale-curriculum-data.js'})));
 return out;
}
function remove(d){d.getElementById('v13-curriculum-scales')?.remove();lastKey=''}
function readStore(k,f){try{const x=JSON.parse(localStorage.getItem(k)||'null');return x==null?f:x}catch(e){return f}}
function saveCanonical(p){
 let db=readStore(STORE,{});if(!db||typeof db!=='object'||Array.isArray(db))db={};
...
function createLessonHandoff(p){
 const token='lh-'+String(p.studentId)+'-'+Date.now()+'-'+Math.random().toString(36).slice(2,8);
 let db=readStore(HANDOFF,{});if(!db||typeof db!=='object'||Array.isArray(db))db={};
 db[token]={version:1,token,studentId:String(p.studentId),studentName:p.studentName,level:p.level,term:p.term,kind:'lesson',curriculum:'scales',items:Array.isArray(p.items)?p.items:[],createdAt:new Date().toISOString()};
 /* Keep only the newest 20 handoffs so this transport store cannot grow indefinitely. */
 const keys=Object.keys(db).sort((a,b)=>String(db[b]?.createdAt||'').localeCompare(String(db[a]?.createdAt||'')));const keep={};keys.slice(0,20).forEach(k=>keep[k]=db[k]);
 localStorage.setItem(HANDOFF,JSON.stringify(keep));
 return token;
}
function navigateToLesson(s,token){
 const url=new URL('./lesson-v13.html',window.location.href);url.searchParams.set('studentId',String(s.id));url.searchParams.set('handoff',token);url.searchParams.set('v','scales-to-lesson-'+Date.now());window.location.assign(url.href);
}
function save(kind,s,it,host){
 const chosen=it.filter((x,i)=>host.querySelector('[data-i="'+i+'"]')?.checked);
 if(!chosen.length)return alert('Select at least one scale.');
 ...
 try{
   saveCanonical(p);
   if(kind==='lesson'){
     const token=createLessonHandoff(p);
     const db=readStore(STORE,{}),saved=db[String(s.id)]?.lesson?.scales;
     ...
     const hand=readStore(HANDOFF,{});if(!hand[token]||String(hand[token].studentId)!==String(s.id)||!Array.isArray(hand[token].items)||hand[token].items.length!==chosen.length)throw new Error('Lesson handoff verification failed');
     navigateToLesson(s,token);
   }else{
     const C=window.parent.VIOLIN_AI_CURRICULUM?.controller;if(C&&typeof C.select==='function')C.select(p);
     localStorage.setItem('VIOLIN_AI_CURRENT_SELECTION_V1',JSON.stringify(p));localStorage.setItem('VIOLIN_AI_LESSON_SELECTION_V1',JSON.stringify(p));
     alert('Selected scales added to Homework.');
   }
...
function render(d){
 const s=currentStudent(d);if(!s){remove(d);return}const it=curriculumItems(s);if(!it.length){remove(d);return}
 const key=String(s.id||s.name)+'|'+String(s.level)+'|'+String(Number(s.term)||1);let host=d.getElementById('v13-curriculum-scales');if(host&&lastKey===key)return;
 ...
 lastKey=key;let h='<div class="v13-scale-box"><h3>🎼 Curriculum Scales</h3><div class="v13-scale-sub">Level '+String(s.level)+' · Term '+String(Number(s.term)||1)+'</div><div class="v13-scale-grid">';
 const groups={};it.forEach((x,i)=>(groups[x.category]??=[]).push([x,i]));
 Object.entries(groups).forEach(([cat,a])=>{h+='<div class="v13-scale-cat"><b>'+cat+'</b>';a.forEach(([x,i])=>h+='<label><input type="checkbox" data-i="'+i+'"><span>'+String(x.title||x.name||x)+'</span></label>');h+='</div>'});
...
frame?.addEventListener('load',()=>{bound=null;lastKey='';bind()});if(frame?.contentDocument)bind();window.VIOLIN_AI_CURRICULUM_BRIDGE={version:'16.0',sync:bind};
})();
