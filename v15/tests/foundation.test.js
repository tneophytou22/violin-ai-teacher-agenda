...
test('programme completion is independent from homework/review', async () => {
  ...
  const item = await programme.createItem({ termId: term.id, curriculumId: 'repertoire-v1', curriculumDomain: 'repertoire', objectId: 'g-major', title: 'G Major', targetWeek: 1 });
  const lesson = await new LessonService(repo).create({ termId: term.id, date: '2026-09-12' });
