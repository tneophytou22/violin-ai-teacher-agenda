...
test('teacher agenda controller keeps UI selection state separate from business services', async () => {
  ...
  assert.equal(controller.snapshot().activeLessonId, lesson.activeLessonId ?? controller.snapshot().activeLessonId);
  ...
  const storedLesson = await repo.get('lessons', controller.snapshot().activeLessonId);
