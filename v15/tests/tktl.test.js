...
test('Teacher Unit Card enforces the locked 10-level / 2-term structure', () => {
  const card = createTeacherUnitCard({
    level: 4,
    term: 1,
    technicalIntent: 'Advanced-intermediate positional stability',
    prerequisites: ['L3T2 positional control'],
    ...
    technicalDomains: ['shifting', 'vibrato', 'extensions', 'double stops'],
    ...
    nextTermDependency: 'Integrate shifting, double stops and emerging spiccato',
    difficultyBand: 'CORE',
    status: 'PROVISIONAL'
  });
  ...
  assert.equal(card.term, 1);
  assert.equal(card.pureTechnical.length, 5);
  assert.equal(card.etudes.length, 5);
  assert.equal(card.repertoire.length, 5);
});

test('Teacher Unit Card rejects Term 3 and incomplete module pools', () => {
  assert.throws(() => createTeacherUnitCard({
    level: 4,
    term: 3,
    technicalIntent: 'x',
    prerequisites: ['x'],
    ...
    technicalDomains: ['x'],
    musicalDomains: ['x'],
    teacherDecisionLogic: ['x'],
    readinessCriteria: ['x'],
    nextTermDependency: 'x'
  }));
  ...
  assert.throws(() => createTeacherUnitCard({
    ...
    prerequisites: ['x'],
    scalesLink: ['x'],
    pureTechnical: ['1', '2', '3', '4'],
    etudes: ['1', '2', '3', '4', '5'],
    repertoire: ['1', '2', '3', '4', '5'],
    technicalDomains: ['x'],
    musicalDomains: ['x'],
    teacherDecisionLogic: ['x'],
    readinessCriteria: ['x'],
