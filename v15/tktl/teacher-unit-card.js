...
const requireExactFive = (value, field) => {
  if (!Array.isArray(value) || value.length !== 5) throw new Error(`${field} requires exactly 5 item(s)`);
  return [...value];
};
...
export function createTeacherUnitCard(input) {
  ...
  const pureTechnical = requireExactFive(input.pureTechnical, 'pureTechnical');
  const etudes = requireExactFive(input.etudes, 'etudes');
  const repertoire = requireExactFive(input.repertoire, 'repertoire');
