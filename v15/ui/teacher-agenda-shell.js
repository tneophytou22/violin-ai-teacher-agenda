        ...
        ${student ? `
          ...
          <section data-view="weekly-agenda">
            ...
            <div data-view="programme-actions">
              ...
              ${!state.activeLessonId ? '<span data-view="lesson-action-hint">Start a lesson below to enable Review selected.</span>' : `<span data-view="lesson-action-hint">Active lesson ready for review.</span><span data-view="lesson-action-status" aria-live="polite">Lesson activity: ${state.reviewedItemIds.length} reviewed</span>`}
            </div>
            ${grouped.map(group => `<section data-domain="${group.domain}"><h3>${group.domain.replace('_', ' ')}</h3><ul>${group.items.map(item => `<li><label><input type="checkbox" data-item="${esc(item.id)}" ${state.selectedItemIds.includes(item.id) ? 'checked' : ''} ${item.status === 'COMPLETED' ? 'disabled' : ''}> ${esc(item.title)}${item.status === 'COMPLETED' ? ' <small>(completed)</small>' : ''}${state.reviewedItemIds.includes(item.id) ? ' <small>(reviewed)</small>' : ''}</label><button type="button" data-carry="${esc(item.id)}" ${item.status === 'COMPLETED' ? 'disabled' : ''}>${item.status === 'COMPLETED' ? 'Completed' : 'Carry to next week'}</button></li>`).join('')}</ul></section>`).join('')}
