      ...
      <section data-v15="teacher-agenda" aria-busy="${state.loading}">
        ...
        ${student ? `
          <section data-view="student-dashboard">
            ...
            ${state.scaleProgress ? '<section data-view="scale-progress" aria-label="Scale Progress and Mastery"><div data-view="scale-progress-header"><div><h2>Scale Progress / Mastery</h2><p>' + state.scaleProgress.completed + '/' + state.scaleProgress.total + ' scale items completed</p></div><strong data-view="scale-mastery">' + state.scaleProgress.masteryPercent + '%</strong></div><div data-view="scale-category-list">' + Object.entries(state.scaleProgress.byCategory).map(([category, summary]) => '<div data-scale-category><div><strong>' + esc(category) + '</strong><span>' + summary.completed + '/' + summary.total + '</span></div><progress max="' + (summary.total || 1) + '" value="' + summary.completed + '"></progress></div>').join('') + '</div><div data-view="scale-progress-footer"><span>' + state.scaleProgress.inProgressCategories + ' categories in progress</span><span>Mastery is based on completed scale requirements.</span></div></section>' : ''}
            </div>
